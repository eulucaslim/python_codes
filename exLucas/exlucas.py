...
while not done:
    ...
    # Caso 2 - Irá exibir os mercados disponiveis e poderá remove-lo    
    elif choice == "2":
        print_mercados()
        ...
        if mercado_num <= len(mercados):
            mercado_name = list(mercados.keys())
            [mercado_num - 1]
    ...
    # Caso 3 - Irá exibir os mercados disponiveis e lista-los 
    elif choice == "3":
        print_mercados()
    
    # Caso 4 - Irá exibir os mercados disponiveis e poderá adicionar um item no mercado escolhido
    elif choice == "4":
        
        print_mercados()
        
        mercado_num = int(input("Digite o numero do Mercado que você deseja adicionar um produto: \n"))
        
        if mercado_num <= len(mercados):
            mercado_name = list(mercados.keys())
            [mercado_num - 1]
            product_name = input("Digite o número do produto: \n")
            mercados[mercado_name]['products'].append(product_name)
            print("Produto Adicionado ao Mercado")
        else:
            print('Número do Mercado Inválido')
    
    # Caso 5 - Irá exibir os mercados disponiveis e poderá remover um item no mercado escolhido
    elif choice == "5":
        ...
        if product_num <= len(mercados[mercado_name]["products"]):
            del mercados[mercado_name]['products']
            [product_num - 1]
            print("Produto Removido com Sucesso!")
        else:
            print("Número do Produto Inválido")
    
    # Caso 6 - Irá listar os mercados disponiveis e poderá exibir os item do mercado escolhido    
    elif choice == "6":
        print_mercados()
